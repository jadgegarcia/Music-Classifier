...
def extract_features(audio_file):
    ...
    # st.write("VAR")
    # st.write(mfccs_var)
    combined_mfccs = [val for pair in zip(mfccs_mean, mfccs_var) for val in pair]

    return [chroma_stft_mean, chroma_stft_var, rms_mean, rms_var, spectral_centroid_mean, spectral_centroid_var, spectral_bandwidth_mean, spectral_bandwidth_var, rolloff_mean, rolloff_var, zero_crossing_rate_mean, zero_crossing_rate_var, harmony_mean, harmony_var, perceptr_mean, perceptr_var, tempo] + combined_mfccs
...
if st.button(':green[Classify Genre]'):
    ...
    # Make prediction
    predict = model.predict(audio_features)
    st.write("Predict: ")
    st.write(predict)
